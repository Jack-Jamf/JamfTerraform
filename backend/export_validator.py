...
def generate_validation_report(validation_result: dict) -> str:
    ...
    Args:
        validation_result: Result from ExportValidator.validate_export()
    ...
    total = validation_result['total_resources']
    
    # Build report
    report = "# Jamf Pro Export Validation Report\n\n"
    report += f"Generated by JamfTerraform Proporter\n\n"
    ...
    report += f"## 🎯 Estimated Success Rate: **{conf_pct}%** {indicator}\n\n"
    
    # Passed checks
    if passed:
        report += f"## ✅ Passed Checks ({len(passed)})\n\n"
        for check in passed:
            report += f"- {check}\n"
        report += "\n"
