...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_efi_integrity_validated'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_efi_integrity_validated'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_efi_integrity_validated passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_efi_integrity_validated failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_efi_integrity_validated failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
## Addresses the following NIST 800-53 controls:
# * N/A
logmessage "Started processing os_hibernate_mode_destroyfvkeyonstandby_enable"
rule_arch=""
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    result_value=$(/usr/bin/osascript -l JavaScript << EOS
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_hibernate_mode_destroyfvkeyonstandby_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_hibernate_mode_destroyfvkeyonstandby_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_hibernate_mode_destroyfvkeyonstandby_enable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_hibernate_mode_destroyfvkeyonstandby_enable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_hibernate_mode_destroyfvkeyonstandby_enable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
## Addresses the following NIST 800-53 controls:
# * IA-5(1)
logmessage "Started processing pwpolicy_lower_case_character_enforce"
rule_arch=""
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('pwpolicy_lower_case_character_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('pwpolicy_lower_case_character_enforce'))["exempt_reason"]
EOS
)
    customref="$(echo "pwpolicy_lower_case_character_enforce" | rev | cut -d ' ' -f 2- | rev)"
    customref="$(echo "$customref" | tr " " ",")"
    if [[ $result_value == "yes" ]]; then
        logmessage "pwpolicy_lower_case_character_enforce passed (Result: $result_value, Expected: \"{'string': 'yes'}\")"
        /usr/bin/defaults write "$audit_plist" pwpolicy_lower_case_character_enforce -dict-add finding -bool NO
        if [[ ! "$customref" == "pwpolicy_lower_case_character_enforce" ]]; then
            /usr/bin/defaults write "$audit_plist" pwpolicy_lower_case_character_enforce -dict-add reference -string "$customref"
        fi
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_lower_case_character_enforce passed (Result: $result_value, Expected: "{'string': 'yes'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            logmessage "pwpolicy_lower_case_character_enforce failed (Result: $result_value, Expected: \"{'string': 'yes'}\")"
            /usr/bin/defaults write "$audit_plist" pwpolicy_lower_case_character_enforce -dict-add finding -bool YES
            if [[ ! "$customref" == "pwpolicy_lower_case_character_enforce" ]]; then
                /usr/bin/defaults write "$audit_plist" pwpolicy_lower_case_character_enforce -dict-add reference -string "$customref"
            fi
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_lower_case_character_enforce failed (Result: $result_value, Expected: "{'string': 'yes'}")"
        else
            logmessage "pwpolicy_lower_case_character_enforce failed (Result: $result_value, Expected: \"{'string': 'yes'}\") - Exemption Allowed (Reason: \"$exempt_reason\")"
            /usr/bin/defaults write "$audit_plist" pwpolicy_lower_case_character_enforce -dict-add finding -bool YES
            if [[ ! "$customref" == "pwpolicy_lower_case_character_enforce" ]]; then
              /usr/bin/defaults write "$audit_plist" pwpolicy_lower_case_character_enforce -dict-add reference -string "$customref"
            fi
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_lower_case_character_enforce failed (Result: $result_value, Expected: "{'string': 'yes'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
    ...
    logmessage "pwpolicy_lower_case_character_enforce does not apply to this architecture"
    /usr/bin/defaults write "$audit_plist" pwpolicy_lower_case_character_enforce -dict-add finding -bool NO
fi
...
## Addresses the following NIST 800-53 controls:
# * IA-5(1)
logmessage "Started processing pwpolicy_upper_case_character_enforce"
rule_arch=""
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('pwpolicy_upper_case_character_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('pwpolicy_upper_case_character_enforce'))["exempt_reason"]
EOS
)
    customref="$(echo "pwpolicy_upper_case_character_enforce" | rev | cut -d ' ' -f 2- | rev)"
    customref="$(echo "$customref" | tr " " ",")"
    if [[ $result_value == "yes" ]]; then
        logmessage "pwpolicy_upper_case_character_enforce passed (Result: $result_value, Expected: \"{'string': 'yes'}\")"
        /usr/bin/defaults write "$audit_plist" pwpolicy_upper_case_character_enforce -dict-add finding -bool NO
        if [[ ! "$customref" == "pwpolicy_upper_case_character_enforce" ]]; then
            /usr/bin/defaults write "$audit_plist" pwpolicy_upper_case_character_enforce -dict-add reference -string "$customref"
        fi
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_upper_case_character_enforce passed (Result: $result_value, Expected: "{'string': 'yes'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            logmessage "pwpolicy_upper_case_character_enforce failed (Result: $result_value, Expected: \"{'string': 'yes'}\")"
            /usr/bin/defaults write "$audit_plist" pwpolicy_upper_case_character_enforce -dict-add finding -bool YES
            if [[ ! "$customref" == "pwpolicy_upper_case_character_enforce" ]]; then
                /usr/bin/defaults write "$audit_plist" pwpolicy_upper_case_character_enforce -dict-add reference -string "$customref"
            fi
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_upper_case_character_enforce failed (Result: $result_value, Expected: "{'string': 'yes'}")"
        else
            logmessage "pwpolicy_upper_case_character_enforce failed (Result: $result_value, Expected: \"{'string': 'yes'}\") - Exemption Allowed (Reason: \"$exempt_reason\")"
            /usr/bin/defaults write "$audit_plist" pwpolicy_upper_case_character_enforce -dict-add finding -bool YES
            if [[ ! "$customref" == "pwpolicy_upper_case_character_enforce" ]]; then
              /usr/bin/defaults write "$audit_plist" pwpolicy_upper_case_character_enforce -dict-add reference -string "$customref"
            fi
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_upper_case_character_enforce failed (Result: $result_value, Expected: "{'string': 'yes'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
    ...
    logmessage "pwpolicy_upper_case_character_enforce does not apply to this architecture"
    /usr/bin/defaults write "$audit_plist" pwpolicy_upper_case_character_enforce -dict-add finding -bool NO
fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_cd_dvd_sharing_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_cd_dvd_sharing_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_cd_dvd_sharing_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_cd_dvd_sharing_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_cd_dvd_sharing_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOF
pwpolicy_dir=$(mktemp -d)
echo "${pwpolicy_xml}" > $pwpolicy_dir/pwpolicy.xml
pwpolicy_file=$pwpolicy_dir/pwpolicy.xml
...
if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
    if [[ $pwpolicy_lower_case_character_enforce_audit_score == "true" ]]; then
        ...
        if [[ $? == 0 ]]; then
            logmessage "Running the command to configure the settings for: pwpolicy_lower_case_character_enforce ..."
            /usr/bin/pwpolicy setaccountpolicies $pwpolicy_file
        fi
    else
        logmessage "Settings for: pwpolicy_lower_case_character_enforce already configured, continuing..."
    fi
elif [[ ! -z "$exempt_reason" ]];then
    logmessage "pwpolicy_lower_case_character_enforce has an exemption, remediation skipped (Reason: "$exempt_reason")"
fi
...
if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
    if [[ $pwpolicy_upper_case_character_enforce_audit_score == "true" ]]; then
        ...
        if [[ $? == 0 ]]; then
            logmessage "Running the command to configure the settings for: pwpolicy_upper_case_character_enforce ..."
            /usr/bin/pwpolicy setaccountpolicies $pwpolicy_file
        fi
    else
        logmessage "Settings for: pwpolicy_upper_case_character_enforce already configured, continuing..."
    fi
elif [[ ! -z "$exempt_reason" ]];then
    logmessage "pwpolicy_upper_case_character_enforce has an exemption, remediation skipped (Reason: "$exempt_reason")"
fi
