...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_firewall_log_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_firewall_log_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_firewall_log_enable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_firewall_log_enable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_firewall_log_enable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_gatekeeper_enable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_gatekeeper_enable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_gatekeeper_enable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_nfsd_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_nfsd_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_nfsd_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_siri_listen_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_siri_listen_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_siri_listen_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_siri_listen_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_siri_listen_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
#####----- Rule: system_settings_sleep_enforce -----#####
## Addresses the following NIST 800-53 controls:
# * N/A
logmessage "Started processing system_settings_sleep_enforce"
rule_arch="arm64"
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    result_value=$(error_count=0
...
echo "$error_count"
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_sleep_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_sleep_enforce'))["exempt_reason"]
EOS
)
    customref="$(echo "system_settings_sleep_enforce" | rev | cut -d ' ' -f 2- | rev)"
    customref="$(echo "$customref" | tr " " ",")"
    if [[ $result_value == "0" ]]; then
        logmessage "system_settings_sleep_enforce passed (Result: $result_value, Expected: \"{'integer': 0}\")"
        /usr/bin/defaults write "$audit_plist" system_settings_sleep_enforce -dict-add finding -bool NO
        if [[ ! "$customref" == "system_settings_sleep_enforce" ]]; then
            /usr/bin/defaults write "$audit_plist" system_settings_sleep_enforce -dict-add reference -string "$customref"
        fi
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_sleep_enforce passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            logmessage "system_settings_sleep_enforce failed (Result: $result_value, Expected: \"{'integer': 0}\")"
            /usr/bin/defaults write "$audit_plist" system_settings_sleep_enforce -dict-add finding -bool YES
            if [[ ! "$customref" == "system_settings_sleep_enforce" ]]; then
                /usr/bin/defaults write "$audit_plist" system_settings_sleep_enforce -dict-add reference -string "$customref"
            fi
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_sleep_enforce failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            logmessage "system_settings_sleep_enforce failed (Result: $result_value, Expected: \"{'integer': 0}\") - Exemption Allowed (Reason: \"$exempt_reason\")"
            /usr/bin/defaults write "$audit_plist" system_settings_sleep_enforce -dict-add finding -bool YES
            if [[ ! "$customref" == "system_settings_sleep_enforce" ]]; then
              /usr/bin/defaults write "$audit_plist" system_settings_sleep_enforce -dict-add reference -string "$customref"
            fi
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_sleep_enforce failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
    ...
    logmessage "system_settings_sleep_enforce does not apply to this architecture"
    /usr/bin/defaults write "$audit_plist" system_settings_sleep_enforce -dict-add finding -bool NO
fi
...
system_settings_sleep_enforce_audit_score=$($plb -c "print system_settings_sleep_enforce:finding" $audit_plist)
if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
    if [[ $system_settings_sleep_enforce_audit_score == "true" ]]; then
        ask 'system_settings_sleep_enforce - Run the command(s)-> /usr/bin/pmset -a sleep 15
/usr/bin/pmset -a displaysleep 10 ' N
        if [[ $? == 0 ]]; then
            logmessage "Running the command to configure the settings for: system_settings_sleep_enforce ..."
            /usr/bin/pmset -a sleep 15
/usr/bin/pmset -a displaysleep 10
        fi
    else
        logmessage "Settings for: system_settings_sleep_enforce already configured, continuing..."
    fi
elif [[ ! -z "$exempt_reason" ]];then
    logmessage "system_settings_sleep_enforce has an exemption, remediation skipped (Reason: "$exempt_reason")"
fi
