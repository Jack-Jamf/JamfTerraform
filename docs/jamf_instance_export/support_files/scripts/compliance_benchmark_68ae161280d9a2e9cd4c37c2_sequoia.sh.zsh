...
audit_plist="/Library/Preferences/com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit.plist"
audit_log="/Library/Logs/compliance_benchmark_68ae161280d9a2e9cd4c37c2_baseline.log"
...
show_menus() {
    lastComplianceScan=$(defaults read /Library/Preferences/com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit.plist lastComplianceCheck)
...
reset_plist(){
    if [[ $reset_all ]];then
        ...
        echo "Clearing results from /Library/Preferences/com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit.plist"
        rm -f /Library/Preferences/com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit.plist
        rm -f /Library/Logs/compliance_benchmark_68ae161280d9a2e9cd4c37c2_baseline.log
    fi
}
...
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey("$rule"))["exempt"]
EOS
)
            if [[ $is_exempt == "1" ]]; then
...
if [[ $(/usr/bin/tail -n 1 "$audit_log" 2>/dev/null) = *"Remediation complete" ]]; then
 	echo "$(date -u) Beginning compliance_benchmark_68ae161280d9a2e9cd4c37c2 baseline scan" >> "$audit_log"
else
 	echo "$(date -u) Beginning compliance_benchmark_68ae161280d9a2e9cd4c37c2 baseline scan" > "$audit_log"
fi
...
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_acls_files_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_acls_files_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_acls_files_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_acls_files_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_acls_files_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_acls_folders_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_acls_folders_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_acls_folders_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_acls_folders_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_acls_folders_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ $LAUNCHD_RUNNING == 1 ]] && [[ -e /etc/security/audit_control ]] && [[ $AUDITD_RUNNING == 1 ]]; then
  ...
  echo "fail"
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_auditd_enabled'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_auditd_enabled'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "pass" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_auditd_enabled passed (Result: $result_value, Expected: "{'string': 'pass'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_auditd_enabled failed (Result: $result_value, Expected: "{'string': 'pass'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_auditd_enabled failed (Result: $result_value, Expected: "{'string': 'pass'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_control_acls_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_control_acls_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_control_acls_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_control_acls_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_control_acls_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_control_group_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_control_group_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_control_group_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_control_group_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_control_group_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_control_mode_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_control_mode_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_control_mode_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_control_mode_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_control_mode_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_control_owner_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_control_owner_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_control_owner_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_control_owner_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_control_owner_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_files_group_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_files_group_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_files_group_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_files_group_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_files_group_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_files_mode_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_files_mode_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_files_mode_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_files_mode_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_files_mode_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_files_owner_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_files_owner_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_files_owner_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_files_owner_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_files_owner_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
# * CM-5(1)
# * MA-4(1)
logmessage "Started processing audit_flags_aa_configure"
rule_arch=""
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_flags_aa_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_flags_aa_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_flags_aa_configure passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_flags_aa_configure failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_flags_aa_configure failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
# * CM-5(1)
# * MA-4(1)
logmessage "Started processing audit_flags_ad_configure"
rule_arch=""
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    result_value=$(/usr/bin/awk -F':' '/^flags/ { print $NF }' /etc/security/audit_control | /usr/bin/tr ',' '\n' | /usr/bin/grep -Ec 'ad'
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_flags_ad_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_flags_ad_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_flags_ad_configure passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_flags_ad_configure failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_flags_ad_configure failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
# * AU-2
# * CM-5(1)
logmessage "Started processing audit_flags_ex_configure"
rule_arch=""
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    result_value=$(/usr/bin/awk -F':' '/^flags/ { print $NF }' /etc/security/audit_control | /usr/bin/tr ',' '\n' | /usr/bin/grep -Ec '\-ex'
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_flags_ex_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_flags_ex_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_flags_ex_configure passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_flags_ex_configure failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_flags_ex_configure failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
# * CM-5(1)
# * MA-4(1)
logmessage "Started processing audit_flags_fm_failed_configure"
rule_arch=""
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    result_value=$(/usr/bin/awk -F':' '/^flags/ { print $NF }' /etc/security/audit_control | /usr/bin/tr ',' '\n' | /usr/bin/grep -Ec '\-fm'
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_flags_fm_failed_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_flags_fm_failed_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_flags_fm_failed_configure passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_flags_fm_failed_configure failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_flags_fm_failed_configure failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
# * CM-5(1)
# * MA-4(1)
logmessage "Started processing audit_flags_fr_configure"
rule_arch=""
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    result_value=$(/usr/bin/awk -F':' '/^flags/ { print $NF }' /etc/security/audit_control | /usr/bin/tr ',' '\n' | /usr/bin/grep -Ec '\-fr'
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_flags_fr_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_flags_fr_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_flags_fr_configure passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_flags_fr_configure failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_flags_fr_configure failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
# * CM-5(1)
# * MA-4(1)
logmessage "Started processing audit_flags_fw_configure"
rule_arch=""
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    result_value=$(/usr/bin/awk -F':' '/^flags/ { print $NF }' /etc/security/audit_control | /usr/bin/tr ',' '\n' | /usr/bin/grep -Ec '\-fw'
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_flags_fw_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_flags_fw_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_flags_fw_configure passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_flags_fw_configure failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_flags_fw_configure failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
# * AU-2
# * MA-4(1)
logmessage "Started processing audit_flags_lo_configure"
rule_arch=""
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    result_value=$(/usr/bin/awk -F':' '/^flags/ { print $NF }' /etc/security/audit_control | /usr/bin/tr ',' '\n' | /usr/bin/grep -Ec '^lo'
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_flags_lo_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_flags_lo_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_flags_lo_configure passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_flags_lo_configure failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_flags_lo_configure failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_folder_group_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_folder_group_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_folder_group_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_folder_group_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_folder_group_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_folder_owner_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_folder_owner_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_folder_owner_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_folder_owner_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_folder_owner_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_folders_mode_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_folders_mode_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "700" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_folders_mode_configure passed (Result: $result_value, Expected: "{'integer': 700}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_folders_mode_configure failed (Result: $result_value, Expected: "{'integer': 700}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_folders_mode_configure failed (Result: $result_value, Expected: "{'integer': 700}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_retention_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('audit_retention_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "60d OR 5G" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_retention_configure passed (Result: $result_value, Expected: "{'string': '60d OR 5G'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_retention_configure failed (Result: $result_value, Expected: "{'string': '60d OR 5G'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - audit_retention_configure failed (Result: $result_value, Expected: "{'string': '60d OR 5G'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
# * CM-7, CM-7(1)
# * SC-7(10)
logmessage "Started processing icloud_sync_disable"
rule_arch=""
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('icloud_sync_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('icloud_sync_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - icloud_sync_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - icloud_sync_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - icloud_sync_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_airdrop_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_airdrop_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_airdrop_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_airdrop_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_airdrop_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_anti_virus_installed'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_anti_virus_installed'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "2" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_anti_virus_installed passed (Result: $result_value, Expected: "{'integer': 2}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_anti_virus_installed failed (Result: $result_value, Expected: "{'integer': 2}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_anti_virus_installed failed (Result: $result_value, Expected: "{'integer': 2}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_authenticated_root_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_authenticated_root_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_authenticated_root_enable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_authenticated_root_enable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_authenticated_root_enable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
## Addresses the following NIST 800-53 controls:
# * CM-7, CM-7(1)
logmessage "Started processing os_bonjour_disable"
rule_arch=""
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    result_value=$(/usr/bin/osascript -l JavaScript << EOS
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_bonjour_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_bonjour_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_bonjour_disable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_bonjour_disable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_bonjour_disable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_config_data_install_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_config_data_install_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_config_data_install_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_config_data_install_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_config_data_install_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_gatekeeper_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_gatekeeper_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_gatekeeper_enable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_gatekeeper_enable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_gatekeeper_enable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_guest_folder_removed'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_guest_folder_removed'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_guest_folder_removed passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_guest_folder_removed failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_guest_folder_removed failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_home_folders_secure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_home_folders_secure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_home_folders_secure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_home_folders_secure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_home_folders_secure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_httpd_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_httpd_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_httpd_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_httpd_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_httpd_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_install_log_retention_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_install_log_retention_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "Yes" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_install_log_retention_configure passed (Result: $result_value, Expected: "{'string': 'Yes'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_install_log_retention_configure failed (Result: $result_value, Expected: "{'string': 'Yes'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_install_log_retention_configure failed (Result: $result_value, Expected: "{'string': 'Yes'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_mail_summary_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_mail_summary_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_mail_summary_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_mail_summary_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_mail_summary_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_mdm_require'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_mdm_require'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_mdm_require passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_mdm_require failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_mdm_require failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_mobile_file_integrity_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_mobile_file_integrity_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_mobile_file_integrity_enable passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_mobile_file_integrity_enable failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_mobile_file_integrity_enable failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$isDisabled" == "disabled" ]] && [[ -z $(/usr/bin/pgrep nfsd) ]]; then
  ...
  echo "fail"
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_nfsd_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_nfsd_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "pass" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_nfsd_disable passed (Result: $result_value, Expected: "{'string': 'pass'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_nfsd_disable failed (Result: $result_value, Expected: "{'string': 'pass'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_nfsd_disable failed (Result: $result_value, Expected: "{'string': 'pass'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_notes_transcription_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_notes_transcription_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_notes_transcription_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_notes_transcription_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_notes_transcription_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_notes_transcription_summary_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_notes_transcription_summary_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_notes_transcription_summary_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_notes_transcription_summary_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_notes_transcription_summary_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_on_device_dictation_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_on_device_dictation_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_on_device_dictation_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_on_device_dictation_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_on_device_dictation_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [ -z "$HINT" ]; then
  ...
  echo "FAIL"
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_password_hint_remove'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_password_hint_remove'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "PASS" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_password_hint_remove passed (Result: $result_value, Expected: "{'string': 'PASS'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_password_hint_remove failed (Result: $result_value, Expected: "{'string': 'PASS'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_password_hint_remove failed (Result: $result_value, Expected: "{'string': 'PASS'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
## Addresses the following NIST 800-53 controls:
# * AC-8
logmessage "Started processing os_policy_banner_loginwindow_enforce"
rule_arch=""
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_policy_banner_loginwindow_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_policy_banner_loginwindow_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_policy_banner_loginwindow_enforce passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_policy_banner_loginwindow_enforce failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_policy_banner_loginwindow_enforce failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_power_nap_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_power_nap_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_power_nap_disable passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_power_nap_disable failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_power_nap_disable failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_root_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_root_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_root_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_root_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_root_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_safari_advertising_privacy_protection_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_safari_advertising_privacy_protection_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_safari_advertising_privacy_protection_enable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_safari_advertising_privacy_protection_enable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_safari_advertising_privacy_protection_enable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_safari_open_safe_downloads_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_safari_open_safe_downloads_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_safari_open_safe_downloads_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_safari_open_safe_downloads_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_safari_open_safe_downloads_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_safari_prevent_cross-site_tracking_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_safari_prevent_cross-site_tracking_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_safari_prevent_cross-site_tracking_enable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_safari_prevent_cross-site_tracking_enable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_safari_prevent_cross-site_tracking_enable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_safari_show_full_website_address_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_safari_show_full_website_address_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_safari_show_full_website_address_enable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_safari_show_full_website_address_enable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_safari_show_full_website_address_enable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_safari_show_status_bar_enabled'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_safari_show_status_bar_enabled'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_safari_show_status_bar_enabled passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_safari_show_status_bar_enabled failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_safari_show_status_bar_enabled failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_safari_warn_fraudulent_website_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_safari_warn_fraudulent_website_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_safari_warn_fraudulent_website_enable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_safari_warn_fraudulent_website_enable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_safari_warn_fraudulent_website_enable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_show_filename_extensions_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_show_filename_extensions_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_show_filename_extensions_enable passed (Result: $result_value, Expected: "{'boolean': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_show_filename_extensions_enable failed (Result: $result_value, Expected: "{'boolean': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_show_filename_extensions_enable failed (Result: $result_value, Expected: "{'boolean': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_sip_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_sip_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_sip_enable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_sip_enable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_sip_enable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
## Addresses the following NIST 800-53 controls:
# * N/A
logmessage "Started processing os_sleep_and_display_sleep_apple_silicon_enable"
rule_arch="arm64"
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
...
echo "$error_count"
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_sleep_and_display_sleep_apple_silicon_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_sleep_and_display_sleep_apple_silicon_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_sleep_and_display_sleep_apple_silicon_enable passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_sleep_and_display_sleep_apple_silicon_enable failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_sleep_and_display_sleep_apple_silicon_enable failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_software_update_deferral'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_software_update_deferral'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_software_update_deferral passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_software_update_deferral failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_software_update_deferral failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_sudo_log_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_sudo_log_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_sudo_log_enforce passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_sudo_log_enforce failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_sudo_log_enforce failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_sudo_timeout_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_sudo_timeout_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_sudo_timeout_configure passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_sudo_timeout_configure failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_sudo_timeout_configure failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_sudoers_timestamp_type_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_sudoers_timestamp_type_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "tty" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_sudoers_timestamp_type_configure passed (Result: $result_value, Expected: "{'string': 'tty'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_sudoers_timestamp_type_configure failed (Result: $result_value, Expected: "{'string': 'tty'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_sudoers_timestamp_type_configure failed (Result: $result_value, Expected: "{'string': 'tty'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_system_wide_applications_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_system_wide_applications_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_system_wide_applications_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_system_wide_applications_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_system_wide_applications_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_terminal_secure_keyboard_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_terminal_secure_keyboard_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_terminal_secure_keyboard_enable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_terminal_secure_keyboard_enable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_terminal_secure_keyboard_enable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_time_server_enabled'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_time_server_enabled'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_time_server_enabled passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_time_server_enabled failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_time_server_enabled failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_unlock_active_user_session_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_unlock_active_user_session_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_unlock_active_user_session_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_unlock_active_user_session_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_unlock_active_user_session_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
## Addresses the following NIST 800-53 controls:
# * N/A
logmessage "Started processing os_world_writable_library_folder_configure"
rule_arch=""
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_world_writable_library_folder_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_world_writable_library_folder_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_world_writable_library_folder_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_world_writable_library_folder_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_world_writable_library_folder_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_world_writable_system_folder_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_world_writable_system_folder_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_world_writable_system_folder_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_world_writable_system_folder_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_world_writable_system_folder_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_writing_tools_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('os_writing_tools_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_writing_tools_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_writing_tools_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - os_writing_tools_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('pwpolicy_account_lockout_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('pwpolicy_account_lockout_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "yes" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_account_lockout_enforce passed (Result: $result_value, Expected: "{'string': 'yes'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_account_lockout_enforce failed (Result: $result_value, Expected: "{'string': 'yes'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_account_lockout_enforce failed (Result: $result_value, Expected: "{'string': 'yes'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('pwpolicy_account_lockout_timeout_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('pwpolicy_account_lockout_timeout_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "yes" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_account_lockout_timeout_enforce passed (Result: $result_value, Expected: "{'string': 'yes'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_account_lockout_timeout_enforce failed (Result: $result_value, Expected: "{'string': 'yes'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_account_lockout_timeout_enforce failed (Result: $result_value, Expected: "{'string': 'yes'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
## Addresses the following NIST 800-53 controls:
# * IA-5(1)
logmessage "Started processing pwpolicy_alpha_numeric_enforce"
rule_arch=""
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('pwpolicy_alpha_numeric_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('pwpolicy_alpha_numeric_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_alpha_numeric_enforce passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_alpha_numeric_enforce failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_alpha_numeric_enforce failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
## Addresses the following NIST 800-53 controls:
# * IA-5(1)
logmessage "Started processing pwpolicy_custom_regex_enforce"
rule_arch=""
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('pwpolicy_custom_regex_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('pwpolicy_custom_regex_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_custom_regex_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_custom_regex_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_custom_regex_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('pwpolicy_history_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('pwpolicy_history_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "yes" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_history_enforce passed (Result: $result_value, Expected: "{'string': 'yes'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_history_enforce failed (Result: $result_value, Expected: "{'string': 'yes'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_history_enforce failed (Result: $result_value, Expected: "{'string': 'yes'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('pwpolicy_max_lifetime_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('pwpolicy_max_lifetime_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "365" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_max_lifetime_enforce passed (Result: $result_value, Expected: "{'integer': 365}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_max_lifetime_enforce failed (Result: $result_value, Expected: "{'integer': 365}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_max_lifetime_enforce failed (Result: $result_value, Expected: "{'integer': 365}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('pwpolicy_minimum_length_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('pwpolicy_minimum_length_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_minimum_length_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_minimum_length_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_minimum_length_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
## Addresses the following NIST 800-53 controls:
# * IA-5(1)
logmessage "Started processing pwpolicy_special_character_enforce"
rule_arch=""
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('pwpolicy_special_character_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('pwpolicy_special_character_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_special_character_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_special_character_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - pwpolicy_special_character_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_airplay_receiver_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_airplay_receiver_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_airplay_receiver_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_airplay_receiver_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_airplay_receiver_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_automatic_login_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_automatic_login_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_automatic_login_disable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_automatic_login_disable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_automatic_login_disable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_bluetooth_menu_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_bluetooth_menu_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "18" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_bluetooth_menu_enable passed (Result: $result_value, Expected: "{'integer': 18}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_bluetooth_menu_enable failed (Result: $result_value, Expected: "{'integer': 18}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_bluetooth_menu_enable failed (Result: $result_value, Expected: "{'integer': 18}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_bluetooth_sharing_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_bluetooth_sharing_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_bluetooth_sharing_disable passed (Result: $result_value, Expected: "{'boolean': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_bluetooth_sharing_disable failed (Result: $result_value, Expected: "{'boolean': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_bluetooth_sharing_disable failed (Result: $result_value, Expected: "{'boolean': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
## Addresses the following NIST 800-53 controls:
# * CM-7, CM-7(1)
logmessage "Started processing system_settings_content_caching_disable"
rule_arch=""
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    result_value=$(/usr/bin/osascript -l JavaScript << EOS
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_content_caching_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_content_caching_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_content_caching_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_content_caching_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_content_caching_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_critical_update_install_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_critical_update_install_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_critical_update_install_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_critical_update_install_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_critical_update_install_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_diagnostics_reports_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_diagnostics_reports_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_diagnostics_reports_disable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_diagnostics_reports_disable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_diagnostics_reports_disable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_external_intelligence_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_external_intelligence_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_external_intelligence_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_external_intelligence_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_external_intelligence_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_external_intelligence_sign_in_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_external_intelligence_sign_in_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_external_intelligence_sign_in_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_external_intelligence_sign_in_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_external_intelligence_sign_in_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$dontAllowDisable" == "true" ]] && [[ "$fileVault" == 1 ]]; then
  ...
  echo "0"
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_filevault_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_filevault_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_filevault_enforce passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_filevault_enforce failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_filevault_enforce failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_firewall_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_firewall_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_firewall_enable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_firewall_enable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_firewall_enable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_firewall_stealth_mode_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_firewall_stealth_mode_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_firewall_stealth_mode_enable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_firewall_stealth_mode_enable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_firewall_stealth_mode_enable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_guest_access_smb_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_guest_access_smb_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_guest_access_smb_disable passed (Result: $result_value, Expected: "{'boolean': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_guest_access_smb_disable failed (Result: $result_value, Expected: "{'boolean': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_guest_access_smb_disable failed (Result: $result_value, Expected: "{'boolean': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_guest_account_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_guest_account_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_guest_account_disable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_guest_account_disable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_guest_account_disable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
## Addresses the following NIST 800-53 controls:
# * AC-11(1)
logmessage "Started processing system_settings_hot_corners_secure"
rule_arch=""
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
...
if [[ "$bl_corner" != "6" ]] && [[ "$tl_corner" != "6" ]] && [[ "$tr_corner" != "6" ]] && [[ "$br_corner" != "6" ]]; then
  echo "0"
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_hot_corners_secure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_hot_corners_secure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_hot_corners_secure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_hot_corners_secure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_hot_corners_secure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_improve_assistive_voice_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_improve_assistive_voice_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_improve_assistive_voice_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_improve_assistive_voice_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_improve_assistive_voice_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_improve_search_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_improve_search_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "2" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_improve_search_disable passed (Result: $result_value, Expected: "{'integer': 2}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_improve_search_disable failed (Result: $result_value, Expected: "{'integer': 2}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_improve_search_disable failed (Result: $result_value, Expected: "{'integer': 2}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_improve_siri_dictation_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_improve_siri_dictation_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "2" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_improve_siri_dictation_disable passed (Result: $result_value, Expected: "{'integer': 2}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_improve_siri_dictation_disable failed (Result: $result_value, Expected: "{'integer': 2}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_improve_siri_dictation_disable failed (Result: $result_value, Expected: "{'integer': 2}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_install_macos_updates_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_install_macos_updates_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_install_macos_updates_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_install_macos_updates_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_install_macos_updates_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_internet_sharing_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_internet_sharing_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_internet_sharing_disable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_internet_sharing_disable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_internet_sharing_disable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
## Addresses the following NIST 800-53 controls:
# * N/A
logmessage "Started processing system_settings_location_services_enable"
rule_arch=""
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_location_services_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_location_services_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_location_services_enable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_location_services_enable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_location_services_enable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_location_services_menu_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_location_services_menu_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_location_services_menu_enforce passed (Result: $result_value, Expected: "{'boolean': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_location_services_menu_enforce failed (Result: $result_value, Expected: "{'boolean': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_location_services_menu_enforce failed (Result: $result_value, Expected: "{'boolean': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_loginwindow_loginwindowtext_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_loginwindow_loginwindowtext_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "Q2VudGVyIGZvciBJbnRlcm5ldCBTZWN1cml0eSBUZXN0IE1lc3NhZ2UK" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_loginwindow_loginwindowtext_enable passed (Result: $result_value, Expected: "{'base64': 'Q2VudGVyIGZvciBJbnRlcm5ldCBTZWN1cml0eSBUZXN0IE1lc3NhZ2UK'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_loginwindow_loginwindowtext_enable failed (Result: $result_value, Expected: "{'base64': 'Q2VudGVyIGZvciBJbnRlcm5ldCBTZWN1cml0eSBUZXN0IE1lc3NhZ2UK'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_loginwindow_loginwindowtext_enable failed (Result: $result_value, Expected: "{'base64': 'Q2VudGVyIGZvciBJbnRlcm5ldCBTZWN1cml0eSBUZXN0IE1lc3NhZ2UK'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_loginwindow_prompt_username_password_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_loginwindow_prompt_username_password_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_loginwindow_prompt_username_password_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_loginwindow_prompt_username_password_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_loginwindow_prompt_username_password_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
# * AC-17
# * AC-3
logmessage "Started processing system_settings_media_sharing_disabled"
rule_arch=""
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    result_value=$(/usr/bin/osascript -l JavaScript << EOS
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_media_sharing_disabled'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_media_sharing_disabled'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_media_sharing_disabled passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_media_sharing_disabled failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_media_sharing_disabled failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_password_hints_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_password_hints_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_password_hints_disable passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_password_hints_disable failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_password_hints_disable failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_personalized_advertising_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_personalized_advertising_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_personalized_advertising_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_personalized_advertising_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_personalized_advertising_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_printer_sharing_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_printer_sharing_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_printer_sharing_disable passed (Result: $result_value, Expected: "{'boolean': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_printer_sharing_disable failed (Result: $result_value, Expected: "{'boolean': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_printer_sharing_disable failed (Result: $result_value, Expected: "{'boolean': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_rae_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_rae_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_rae_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_rae_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_rae_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_remote_management_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_remote_management_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_remote_management_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_remote_management_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_remote_management_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_screen_sharing_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_screen_sharing_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_screen_sharing_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_screen_sharing_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_screen_sharing_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_screensaver_ask_for_password_delay_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_screensaver_ask_for_password_delay_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_screensaver_ask_for_password_delay_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_screensaver_ask_for_password_delay_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_screensaver_ask_for_password_delay_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_screensaver_timeout_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_screensaver_timeout_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_screensaver_timeout_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_screensaver_timeout_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_screensaver_timeout_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_siri_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_siri_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "false" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_siri_disable passed (Result: $result_value, Expected: "{'string': 'false'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_siri_disable failed (Result: $result_value, Expected: "{'string': 'false'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_siri_disable failed (Result: $result_value, Expected: "{'string': 'false'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_smbd_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_smbd_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_smbd_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_smbd_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_smbd_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_software_update_app_update_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_software_update_app_update_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_software_update_app_update_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_software_update_app_update_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_software_update_app_update_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_software_update_download_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_software_update_download_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_software_update_download_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_software_update_download_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_software_update_download_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_software_update_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_software_update_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_software_update_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_software_update_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_software_update_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ $softwareupdate_date_epoch -lt $thirty_days_epoch ]]; then
  ...
  /bin/echo "1"
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_softwareupdate_current'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_softwareupdate_current'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_softwareupdate_current passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_softwareupdate_current failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_softwareupdate_current failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_ssh_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_ssh_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_ssh_disable passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_ssh_disable failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_ssh_disable failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
echo $result
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_system_wide_preferences_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_system_wide_preferences_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "1" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_system_wide_preferences_configure passed (Result: $result_value, Expected: "{'integer': 1}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_system_wide_preferences_configure failed (Result: $result_value, Expected: "{'integer': 1}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_system_wide_preferences_configure failed (Result: $result_value, Expected: "{'integer': 1}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
## Addresses the following NIST 800-53 controls:
# * N/A
logmessage "Started processing system_settings_time_machine_auto_backup_enable"
rule_arch=""
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    unset result_value
    result_value=$(/usr/bin/osascript -l JavaScript << EOS
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_time_machine_auto_backup_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_time_machine_auto_backup_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_time_machine_auto_backup_enable passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_time_machine_auto_backup_enable failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_time_machine_auto_backup_enable failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
echo "$error_count"
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_time_machine_encrypted_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_time_machine_encrypted_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_time_machine_encrypted_configure passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_time_machine_encrypted_configure failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_time_machine_encrypted_configure failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_time_server_configure'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_time_server_configure'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "time.apple.com" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_time_server_configure passed (Result: $result_value, Expected: "{'string': 'time.apple.com'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_time_server_configure failed (Result: $result_value, Expected: "{'string': 'time.apple.com'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_time_server_configure failed (Result: $result_value, Expected: "{'string': 'time.apple.com'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_time_server_enforce'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_time_server_enforce'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "true" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_time_server_enforce passed (Result: $result_value, Expected: "{'string': 'true'}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_time_server_enforce failed (Result: $result_value, Expected: "{'string': 'true'}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_time_server_enforce failed (Result: $result_value, Expected: "{'string': 'true'}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
if [[ "$arch" == "$rule_arch" ]] || [[ -z "$rule_arch" ]]; then
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_wake_network_access_disable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_wake_network_access_disable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "0" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_wake_network_access_disable passed (Result: $result_value, Expected: "{'integer': 0}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_wake_network_access_disable failed (Result: $result_value, Expected: "{'integer': 0}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_wake_network_access_disable failed (Result: $result_value, Expected: "{'integer': 0}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
...
EOS
    ...
    exempt=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_wifi_menu_enable'))["exempt"]
EOS
)
    exempt_reason=$(/usr/bin/osascript -l JavaScript << EOS 2>/dev/null
ObjC.unwrap($.NSUserDefaults.alloc.initWithSuiteName('com.jamf.compliance_benchmark_68ae161280d9a2e9cd4c37c2.audit').objectForKey('system_settings_wifi_menu_enable'))["exempt_reason"]
EOS
)
    ...
    if [[ $result_value == "18" ]]; then
        ...
        /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_wifi_menu_enable passed (Result: $result_value, Expected: "{'integer': 18}")"
    else
        if [[ ! $exempt == "1" ]] || [[ -z $exempt ]];then
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_wifi_menu_enable failed (Result: $result_value, Expected: "{'integer': 18}")"
        else
            ...
            /usr/bin/logger "mSCP: compliance_benchmark_68ae161280d9a2e9cd4c37c2 - system_settings_wifi_menu_enable failed (Result: $result_value, Expected: "{'integer': 18}") - Exemption Allowed (Reason: "$exempt_reason")"
            /bin/sleep 1
        fi
    fi
